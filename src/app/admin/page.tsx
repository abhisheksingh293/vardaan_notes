  ...
  if (!session) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-black p-6 font-sans">
        <form onSubmit={login} className="bg-white dark:bg-zinc-900/50 backdrop-blur-xl p-10 rounded-[2.5rem] shadow-2xl border border-white dark:border-white/5 flex flex-col gap-6 w-full max-w-md animate-in fade-in zoom-in-95 duration-500">
          ...
          <div className="space-y-4">
            <div className="group relative">
              <input 
                type="email" placeholder="Admin Email" className="w-full p-4.5 bg-zinc-100 dark:bg-white/5 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 text-zinc-900 dark:text-white transition-all border border-transparent focus:border-blue-500"
          ...
          <button type="submit" disabled={status.loading} className="w-full bg-blue-600 text-white p-4.5 rounded-2xl font-bold mt-2 hover:bg-blue-700 active:scale-[0.98] transition-all shadow-xl shadow-blue-500/20 disabled:opacity-50">
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#09090b] flex font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col relative overflow-y-auto pb-24 lg:pb-0 h-screen scrollbar-hide">
        ...
        <div className="px-8 pb-10 space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-700">
          {status.success && <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 rounded-2xl text-sm font-bold animate-in zoom-in-95">{status.success}</div>}
          {status.error && <div className="p-4 bg-red-500/10 border border-red-500/20 text-red-500 rounded-2xl text-sm font-bold animate-in zoom-in-95">{status.error}</div>}
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {filteredStudents.map((student, idx) => (
                         <div 
                            ...
                            className="bg-white dark:bg-white/[0.02] p-6 rounded-[2.5rem] border border-slate-200 dark:border-white/[0.05] hover:border-blue-500/30 transition-all hover:shadow-2xl hover:shadow-blue-500/10 group active:scale-[0.98] relative overflow-hidden cursor-pointer"
                         >
                            <div className="absolute top-0 right-0 p-6 opacity-0 group-hover:opacity-100 transition-opacity">
                               <div className={`w-3 h-3 rounded-full ${student.board?.toUpperCase().includes('CBSE') ? 'bg-indigo-500' : 'bg-amber-500'} blur-sm animate-pulse`}></div>
                            </div>

                            <div className="flex flex-col items-center text-center space-y-4 pt-2">
                               <div className={`w-20 h-20 ${getAvatarColor(student.full_name)} rounded-3xl flex items-center justify-center text-white font-black text-3xl shadow-2xl relative group-hover:rotate-6 transition-transform duration-500`}>
                                  <div className="absolute inset-0 bg-white/10 group-hover:bg-white/25 rounded-3xl transition-colors"></div>
                               ...
                               <div className="w-full space-y-3 pt-6">
                                  <div className="p-3 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-white/10 group/url relative">
                                     ...
                                     <div className="flex items-center justify-between gap-2 overflow-hidden">
                                        ...
                                        <button 
                                          ...
                                          className="p-1.5 hover:bg-blue-600 hover:text-white rounded-lg transition-all shrink-0"
                                        >
                                  ...
                                  <button 
                                    ...
                                    className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 transition-all active:scale-95 shadow-lg shadow-blue-500/20"
                                  >
                                     Explore Repository
                                  </button>
                               </div>
                    ...
                    {isTreeLoading ? (
                      ...
                      <div className="space-y-12 pb-20 p-8 lg:p-12">
                         <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                            <div className="space-y-4">
                               <button onClick={() => setSelectedStudentTree(null)} className="flex items-center gap-2 text-zinc-500 hover:text-blue-500 transition-colors group px-1">
                                  ...
                                  <div className="space-y-2 py-2">
                                     ...
                                     <div className="flex flex-col gap-1.5">
                                        ...
                                        <div className="pt-2 flex items-center gap-3">
                                           ...
                                           >
                                              <ExternalLink className="w-3.5 h-3.5" /> Launch Dashboard: <span className="underline underline-offset-4 decoration-2 decoration-blue-500/30 group-hover/link:decoration-blue-500 transition-all font-mono lowercase">
                                                 {(() => {
                                                   const student = students.find(s => s.student_code === selectedStudentTree.folderName.split('_').pop());
                                           ...
                                           <button 
                                              ...
                                              className="p-1.5 bg-blue-500/5 hover:bg-blue-500 text-zinc-500 hover:text-white rounded-lg transition-all border border-blue-500/10"
                                           >
                                              <Link2 className="w-3 h-3" />
                                           </button>
                                        </div>
                                      ...
                                      <div key={testName} className="p-6 bg-slate-50 dark:bg-white/[0.01] rounded-[2rem] border border-slate-200 dark:border-white/[0.05] group/test">
                                        <div className="flex items-center justify-between mb-4">
                                          ...
                                          <button 
                                            ...
                                            className="opacity-0 group-hover/test:opacity-100 p-1.5 hover:bg-red-500/10 hover:text-red-500 rounded-lg transition-all"
                                          >
                                        ...
                                        <div className="space-y-2">
                                          {files.map((file: string) => (
                                            <div key={file} className="flex items-center justify-between p-3.5 bg-white dark:bg-black/20 rounded-xl border border-slate-100 dark:border-white/5 group/file">
                                              <div className="flex items-center gap-3">
                                                <FileText className="w-4 h-4 text-zinc-400 group-hover/file:text-blue-500 transition-colors" />
                                      ...
                                      >
                                        <div className="flex items-center gap-4">
                                          ...
                                          <button 
                                            ...
                                            className="ml-2 p-2 hover:bg-red-500/10 hover:text-red-500 rounded-xl transition-all opacity-0 group-hover:opacity-100"
                                          >
                                            <Trash2 className="w-4 h-4" />
                                          </button>
                                        </div>
                                          ...
                                          {Object.entries(chapters).map(([chap, files]: [string, any]) => (
                                            <div key={chap} className="space-y-1">
                                              <div 
                                                ...
                                                className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 transition-all group/chap cursor-pointer"
                                              >
                                                ...
                                                <ChevronRight className={`w-3.5 h-3.5 transition-transform duration-300 ${expandedFolders[`${sub}-${chap}`] ? 'rotate-90 text-blue-500' : 'opacity-30'}`} />
                                              </div>
                                              
                                              {expandedFolders[`${sub}-${chap}`] && (
                                                <div className="pl-6 space-y-2 pt-1">
                                                  {files.map((file: string) => (
                                                    <div key={file} className="flex items-center justify-between gap-3 text-[11px] font-bold text-zinc-400 py-1.5 border-l border-slate-100 dark:border-white/5 pl-4 group/file hover:text-blue-500/80 transition-colors">
                                                      ...
                                                      <div className="flex items-center gap-1.5 mr-2">
                                                        <button 
                                                          ...
                                                          className="opacity-0 group-hover/file:opacity-100 hover:text-blue-600 transition-all p-1"
                                                        >
                                                        ...
                                                        <button 
                                                          ...
                                                          className="opacity-0 group-hover/file:opacity-100 hover:text-red-500 transition-all p-1"
                                                        >
               ...
               <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-zinc-200 dark:border-white/5">
                  ...
                  <div className="flex gap-4">
                     <button 
                        ...
                        className="px-8 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-[1.5rem] font-black text-xs uppercase tracking-widest shadow-xl shadow-indigo-500/20 active:scale-95 transition-all flex items-center gap-3 disabled:opacity-50"
                     >
                     ...
                     <button 
                        ...
                        disabled={status.loading}
                        className="px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-[1.5rem] font-black text-xs uppercase tracking-widest shadow-xl shadow-blue-500/20 active:scale-95 transition-all flex items-center gap-3 disabled:opacity-50"
                     >
          ...
          {/* TAB: Deployer */}
          {activeTab === 'deployer' && (
            <div className="max-w-3xl mx-auto py-8 space-y-12 animate-in fade-in zoom-in-95 duration-700">
               {/* Console Header */}
               ...
               <div className="bg-white dark:bg-zinc-900/40 backdrop-blur-3xl p-10 rounded-[3rem] border border-zinc-200 dark:border-white/5 shadow-2xl space-y-10 relative overflow-hidden">
                  ...
                  <div className="space-y-8 relative">
                      ...
                      <div className="space-y-4">
                        ...
                        {isCreatingNewChapter && (
                          <input 
                            ...
                            className="w-full py-5 px-6 bg-blue-500/5 dark:bg-blue-500/10 border-2 border-blue-500/30 rounded-[1.8rem] outline-none focus:ring-0 text-zinc-900 dark:text-white animate-in slide-in-from-top-4 font-black text-sm tracking-tight placeholder:opacity-30"
                            value={form.chapter} 
                    ...
                    {form.studentCode && form.subject && form.chapter && (
                       <div className="mt-4 p-5 bg-zinc-900/80 rounded-[2rem] border border-white/5 animate-in fade-in slide-in-from-bottom-2">
                  ...
                  <button 
                    ...
                    className="w-full py-6 bg-blue-600 hover:bg-blue-700 text-white rounded-[2rem] font-black text-lg shadow-2xl shadow-blue-500/30 disabled:opacity-30 active:scale-[0.98] transition-all flex items-center justify-center gap-4 relative group overflow-hidden"
                  >
            ...
            <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in duration-700">
               ...
               <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 border-b border-slate-100 dark:border-white/5 pb-10">
                  ...
                  <div className="relative group min-w-[320px]">
                     <Search className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-zinc-400 group-focus-within:text-blue-500 transition-all duration-300" />
                     <input 
                        type="text" 
                        placeholder="Live Filter Master List..."
                        className="w-full pl-14 pr-6 py-4.5 bg-white dark:bg-white/[0.03] rounded-[1.5rem] border border-slate-200 dark:border-white/10 outline-none focus:ring-2 focus:ring-blue-500 shadow-sm text-sm font-bold transition-all transition-all"
               ...
               <div className="grid grid-cols-1 gap-4">
                  <div className="flex items-center justify-between pb-2">
                     ...
                     <button 
                        ...
                        className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-blue-500/20 active:scale-95 transition-all flex items-center gap-2"
                     >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                     {globalSubjects[activeConfigBoard]
                        ...
                        <div 
                           key={subject} 
                           className="flex items-center justify-between p-5 bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.05] rounded-[2rem] hover:border-blue-500/30 transition-all shadow-sm hover:shadow-2xl hover:shadow-blue-500/5 group"
                        >
                           <div className="flex items-center gap-4 flex-1 min-w-0">
                              <ModernSubjectIcon name={subject} />
                              <span className="font-black text-sm tracking-tight capitalize dark:text-zinc-200 group-hover:text-blue-500 transition-colors truncate">{subject}</span>
                           </div>
                           
                           <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all ml-4 shrink-0">
                              <button 
                                 onClick={async () => {
                                 ...
                                 className="w-9 h-9 bg-zinc-100 dark:bg-white/5 hover:bg-blue-500/10 hover:text-blue-500 rounded-xl flex items-center justify-center transition-colors"
                              >
                              ...
                              <button 
                                 ...
                                 className="w-9 h-9 bg-zinc-100 dark:bg-white/5 hover:bg-red-500/10 hover:text-red-500 rounded-xl flex items-center justify-center transition-colors"
                              >
                                 <Trash2 className="w-4 h-4" />
                              </button>
                           </div>
...
function NavItem({ active, onClick, icon, label }: any) {
  return (
    <button 
      onClick={onClick}
      className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all duration-300 font-bold outline-none border-none ${
        active 
          ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20 active:scale-[0.98]' 
          : 'text-zinc-500 hover:bg-slate-50 dark:hover:bg-white/[0.03] hover:text-zinc-800 dark:hover:text-white'
      }`}
    >
      <span className={active ? 'text-white' : 'text-zinc-400 group-hover:text-blue-500'}>{icon}</span>
...
function StatCard({ label, value, icon, color }: any) {
  return (
    <div className="bg-white dark:bg-white/[0.02] p-8 rounded-[2rem] border border-slate-200 dark:border-white/[0.05] shadow-xl shadow-slate-200/50 dark:shadow-none transition-all hover:-translate-y-1">
  ...
  return (
    <div className="relative">
      <button 
        ...
        className={`w-full py-5 px-6 bg-slate-50 dark:bg-white/[0.03] border border-zinc-200 dark:border-white/10 rounded-[1.8rem] text-left outline-none focus:ring-4 focus:ring-blue-500/20 text-zinc-900 dark:text-white transition-all font-black text-sm tracking-tight flex items-center justify-between disabled:opacity-30`}
      >
        <span className={!selected ? "opacity-30" : ""}>
          {selected ? selected.label : placeholder}
        </span>
        <ChevronRight className={`w-4 h-4 transition-transform duration-300 ${isOpen ? 'rotate-90' : 'opacity-40'}`} />
      </button>

      {isOpen && !disabled && (
        <>
          <div className="fixed inset-0 z-30" onClick={() => setIsOpen(false)} />
          <div className="absolute top-full left-0 right-0 mt-3 bg-white dark:bg-[#18181b]/95 backdrop-blur-3xl border border-zinc-200 dark:border-white/10 rounded-[2rem] shadow-2xl z-40 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300">
            <div className="max-h-60 overflow-y-auto p-2 py-4 custom-scrollbar">
              {options.map((opt: any) => (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-2xl transition-all font-bold text-xs uppercase tracking-tight ${
                    value === opt.value 
                    ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' 
